	...
	protected void createAbstract() {
		try (var stack = MemoryStack.stackPush()) {
			...
			for (int i = 0; i < this.attachments.size(); ++i) {
				...
				pAttachment.set(0, attachment.format, attachment.samples, attachment.loadOp, attachment.storeOp,
						attachment.stencilLoadOp, attachment.stencilStoreOp, attachment.initialLayout,
			...
			for (int i = 0; i < this.subpasses.size(); ++i) {
				...
				if (subpass.useDepthStencilAttachment) {
					...
					pDepthStencilAttachment.set(subpass.depthStencilAttachment.attachment,
				...
				pSubpass.set(0, subpass.pipelineBindPoint, pInputAttachments, subpass.colorAttachmentRefs.size(),
						pColorAttachments, pResolveAttachments, pDepthStencilAttachment, pPreserveAttachments);
			}
			...
			for (int i = 0; i < this.dependencies.size(); ++i) {
				...
				pDependency.set(dependency.srcSubpass, dependency.dstSubpass, dependency.srcStageMask,
			...
			for (var subpass : pSubpasses) {
				...
				if (subpass.pResolveAttachments() != null)
					subpass.pResolveAttachments().free();
				if (subpass.pDepthStencilAttachment() != null)
					subpass.pDepthStencilAttachment().free();
