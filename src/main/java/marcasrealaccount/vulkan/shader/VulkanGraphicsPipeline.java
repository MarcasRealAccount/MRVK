...
public class VulkanGraphicsPipeline extends VulkanPipeline {
	...
	public VulkanGraphicsPipeline(VulkanDevice device, VulkanPipelineLayout pipelineLayout,
	...
	protected void createAbstract() {
		try (var stack = MemoryStack.stackPush()) {
			...
			var pVertexAttributes = VkVertexInputAttributeDescription
					.malloc(this.vertexInputState.vertexAttributes.size());
			for (int i = 0; i < this.vertexInputState.vertexAttributes.size(); ++i) {
				...
				pVertexAttribute.set(vertexAttribute.location, vertexAttribute.binding, vertexAttribute.format,
						vertexAttribute.offset);
			}

			vertexInputState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO, 0, 0, pVertexBindings,
					pVertexAttributes);
			...
			tesselationState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_TESSELLATION_STATE_CREATE_INFO, 0, 0,
					this.tesselationState.patchControlPoints);
			...
			viewportState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO, 0, 0, pViewports.capacity(),
					pViewports, pScissors.capacity(), pScissors);

			rasterizationState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO, 0, 0,
					this.rasterizationState.depthClampEnable, this.rasterizationState.rasterizerDiscardEnable,
					this.rasterizationState.polygonMode.getValue(), this.rasterizationState.cullMode.getValue(),
					this.rasterizationState.frontFace.getValue(), this.rasterizationState.depthBiasEnable,
					this.rasterizationState.depthBiasConstantFactor, this.rasterizationState.depthBiasClamp,
			...
			if (this.multisampleState.sampleMask != null) {
				...
				pSampleMask.put(0, this.multisampleState.sampleMask, 0,
						Math.min(pSampleMask.capacity(), this.multisampleState.sampleMask.length));
			}

			multisampleState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_MULTISAMPLE_STATE_CREATE_INFO, 0, 0,
					this.multisampleState.rasterizationSamples, this.multisampleState.sampleShadingEnable,
					this.multisampleState.minSampleShading, pSampleMask, this.multisampleState.alphaToCoverageEnable,
					this.multisampleState.alphaToOneEnable);

			var depthStencilFrontState = this.depthStencilState.front;
			depthStencilFront.set(depthStencilFrontState.failOp, depthStencilFrontState.passOp,
					depthStencilFrontState.depthFailOp, depthStencilFrontState.compareOp,
					depthStencilFrontState.compareMask, depthStencilFrontState.writeMask,
					depthStencilFrontState.reference);

			var depthStencilBackState = this.depthStencilState.back;
			depthStencilBack.set(depthStencilBackState.failOp, depthStencilBackState.passOp,
					depthStencilBackState.depthFailOp, depthStencilBackState.compareOp,
					depthStencilBackState.compareMask, depthStencilBackState.writeMask,
					depthStencilBackState.reference);

			depthStencilState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO, 0, 0,
					this.depthStencilState.depthTestEnable, this.depthStencilState.depthWriteEnable,
					this.depthStencilState.depthCompareOp, this.depthStencilState.depthBoundsTestEnable,
					this.depthStencilState.stencilTestEnable, depthStencilFront, depthStencilBack,
					this.depthStencilState.minDepthBounds, this.depthStencilState.maxDepthBounds);

			var pColorBlendAttachments = VkPipelineColorBlendAttachmentState
					.malloc(this.colorBlendState.attachments.size());
			for (int i = 0; i < this.colorBlendState.attachments.size(); ++i) {
				...
				pColorBlendAttachment.set(colorBlendAttachment.blendEnable, colorBlendAttachment.srcColorBlendFactor,
						colorBlendAttachment.dstColorBlendFactor, colorBlendAttachment.colorBlendOp,
						colorBlendAttachment.srcAlphaBlendFactor, colorBlendAttachment.dstAlphaBlendFactor,
						colorBlendAttachment.alphaBlendOp, colorBlendAttachment.colorWriteMask);
			}

			colorBlendConstants.put(0, this.colorBlendState.constants, 0,
					Math.min(4, this.colorBlendState.constants.length));

			colorBlendState.set(VK12.VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO, 0, 0,
					this.colorBlendState.logicOpEnable, this.colorBlendState.logicOp, pColorBlendAttachments,
					colorBlendConstants);

			var pDynamicStates = MemoryUtil.memAllocInt(this.dynamicState.states.size());
			for (int i = 0; i < this.dynamicState.states.size(); ++i)
				pDynamicStates.put(i, this.dynamicState.states.get(i));
			...
			createInfo.set(VK12.VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO, 0, 0, stages, vertexInputState,
					inputAssemblyState, tesselationState, viewportState, rasterizationState, multisampleState,
					depthStencilState, colorBlendState, dynamicState, this.pipelineLayout.getHandle(),
	...
	public static class VulkanShaderStage {
		...
		public enum EShaderStage {
			...
			TESSELATION_EVALUATION(VK12.VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT),
			GEOMETRY(VK12.VK_SHADER_STAGE_GEOMETRY_BIT), FRAGMENT(VK12.VK_SHADER_STAGE_FRAGMENT_BIT),
			COMPUTE(VK12.VK_SHADER_STAGE_COMPUTE_BIT), RT_RAYGEN(KHRRayTracingPipeline.VK_SHADER_STAGE_RAYGEN_BIT_KHR),
			RT_ANY_HIT(KHRRayTracingPipeline.VK_SHADER_STAGE_ANY_HIT_BIT_KHR),
			RT_CLOSEST_HIT(KHRRayTracingPipeline.VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR),
			RT_MISS(KHRRayTracingPipeline.VK_SHADER_STAGE_MISS_BIT_KHR),
	...
	public static class ColorBlendState {
		...
		public static class Attachment {
			...
			public int colorWriteMask = VK12.VK_COLOR_COMPONENT_R_BIT | VK12.VK_COLOR_COMPONENT_G_BIT
