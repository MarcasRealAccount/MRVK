...
public class VulkanSwapchain extends VulkanHandle<Long> {
	...
	protected void createAbstract() {
		...
		try (var stack = MemoryStack.stackPush()) {
			...
			createInfo.set(KHRSwapchain.VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR, 0, 0, this.surface.getHandle(),
					this.imageCount, this.format, this.colorSpace, pExtent, this.imageArrayLayers, this.imageUsage,
					imageSharingMode, pIndices, this.preTransform, this.compositeAlpha, this.presentMode, this.clipped,
					this.handle);
			...
			if (KHRSwapchain.vkCreateSwapchainKHR(this.device.getHandle(), createInfo, null,
				...
				for (int i = 0; i < pImages.capacity(); ++i)
					images.add(new VulkanImage(this.device, pImages.get(i)));

				MemoryUtil.memFree(pImages);
			}

			if (pIndices != null)
				MemoryUtil.memFree(pIndices);
		}
	}
